      ...
      <b-row class="mt-2">
        <b-col class="text-center">
          <b-button variant="primary" @click="saveForm"
            >{{ pm_id ? "Update" : "Add" }} Payment</b-button
          >
...
import { GetAllPayment, addEditPayment } from "@/apiServices/DashboardServices";
...
export default {
  ...
  beforeMount() {
    const { pm_id } = this.$route.params;
    this.pm_id = pm_id || null;
    if (pm_id) {
      this.onGetAllUsers();
    }
  },
  ...
  methods: {
    ...
    saveForm() {
      this.$refs.loginValidation.validate().then(async (success) => {
        if (success) {
          try {
            const response = await addEditPayment({
              ...this.form,
              pm_id: this.pm_id || "",
            });
            const { data } = response;
