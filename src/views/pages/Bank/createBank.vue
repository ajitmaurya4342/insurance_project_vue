...
import { GetAllBank, addEditBank } from "@/apiServices/DashboardServices";
...
export default {
  ...
  beforeMount() {
    const { bd_id } = this.$route.params;
    this.bd_id = bd_id || null;
    if (bd_id) {
      this.onGetAllUsers();
    }
  },
  ...
  methods: {
    ...
    saveForm() {
      this.$refs.loginValidation.validate().then(async (success) => {
        if (success) {
          try {
            const response = await addEditBank({
              ...this.form,
              bd_id: this.bd_id || "",
            });
            const { data } = response;
